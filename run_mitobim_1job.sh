# this script should be run like `bash run_mitobim.sh <gene prefix (str)> <missmatch number (int)> <queue> <run time (hr, 00)>`
genes=$1
missmatch=$2
queue=$3
hours=$4
...
do
	...
	if [ ${reads} == Etricolor_SC027-SC029GLS-EtriBMGL_all ]; then
		...
		echo "cd ${dir}; /work/02576/rdtarvin/lonestar/MITObim/MITObim_1.8.pl --mirapath /work/02576/rdtarvin/lonestar/mira/bin/ -end 100 --verbose -sample ${reads} -ref ${genes} -readpool /scratch/02576/rdtarvin/outgroups/${prefix}.fastq --clean -kbait ${kbait} --missmatch ${missmatch} --quick $WORK/gene_dbs/${genes}.fa 2> ${reads}_stderr.log 1> ${reads}_stdout.log" >> submission_$(date +"%Y_%m_%d")_m${missmatch}.job
		echo "##### making submission script for ${reads} using ${genes} genes with kbait of ${kbait} and missmatch parameter of ${missmatch} for ${hours}"	
	fi
...
launcher_creator.py -n ${genes}_batch-mito -t ${hours}:00:00 -q ${queue} -j submission_$(date +"%Y_%m_%d")_m${missmatch}.job
